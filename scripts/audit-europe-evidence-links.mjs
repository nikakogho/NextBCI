...
const tempDir = join(tmpdir(), `nextbci-europe-links-${Date.now()}-${Math.random().toString(36).slice(2)}`);
await Promise.all([mkdir(cacheDir, { recursive: true }), mkdir(tempDir, { recursive: true })]);

let europe;
try {
  const sourcePath = resolve(projectRoot, "data", "europe-evidence.ts");
  ...
  await writeFile(resolve(tempDir, "europe-evidence.js"), output.outputText, "utf8");
  europe = createRequire(resolve(tempDir, "audit.cjs"))("./europe-evidence.js");
} finally {
...
const externalVerificationNotes = new Map([
...
const markdown = `# European evidence source-link audit\n\n` +
...
await writeFile(resolve(projectRoot, "docs", "european-evidence-link-audit.md"), markdown, "utf8");
